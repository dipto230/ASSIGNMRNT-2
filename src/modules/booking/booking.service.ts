...
export const bookingService = {
  createBooking: async (customer_id: number, vehicle_id: number, rent_start_date: string, rent_end_date: string) => {
    ...
    // create booking
    const booking = await bookingModel.createBooking(customer_id, vehicle_id, rent_start_date, rent_end_date, total_price);
  ...
  cancelBooking: async (bookingId: string, user: any) => {
    const booking = await bookingModel.getBookingById(bookingId);
    if (!booking) throw new Error("Booking not found");
    if (user.role !== "admin" && booking.customer_id !== user.id) throw new Error("Forbidden");
    
    const today = new Date();
    if (new Date(booking.rent_start_date) <= today) throw new Error("Cannot cancel after booking start");
  ...
  markAsReturned: async (bookingId: string) => {
    const booking = await bookingModel.getBookingById(bookingId);
    if (!booking) throw new Error("Booking not found");
